...
(() => {
  ...
  function card(it, hit) {
    ...
    const gem = window.CSRPGems
      ? window.CSRPGems.badgeFor({ name: it.name, seed, skin_index: skinIndex, item_id: (hit && hit.imageId) != null ? hit.imageId : smallId(it) })
      : null;
  ...
  async function buildItemMap(t) {
    ...
    const maps = await Promise.all(ids.map(async (id) => {
      ...
      for (const raw of inv) {
        const rec = { imageId: smallId(raw), float: num(raw.float), seed: num(raw.seed), skinIndex: num(raw.skin_index) };
