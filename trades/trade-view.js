(() => {
  ...
  function card(it, hit) {
    ...
    const floatBadge = w ? `<span class="c-float" style="color:${w.c};border-color:${w.c}55"><span class="c-dot" style="background:${w.c}"></span>${w.code} · ${Number(fl).toFixed(4)}</span>` : '<span class="c-float" style="color:var(--muted);border-color:var(--line)">float n/a</span>';
    const seedBadge = seed != null ? `<span class="c-seed">#${Math.round(seed)}</span>` : "";
    const skinIndex = hit && hit.skinIndex != null ? hit.skinIndex : num(it.skin_index);
    const gem = window.CSRPGems ? window.CSRPGems.badgeFor({
      name: it.name,
      seed,
      skin_index: skinIndex,
      item_id: (hit && hit.imageId) != null ? hit.imageId : smallId(it)
    }) : null;
  ...
  function sideBlock(label, tag, items, coins, itemMap) {
    ...
    if (!items.length && !coins) grid.append(el("div", "empty", "Nothing on this side.")); else items.forEach(it => grid.append(card(it, lookupHit(itemMap, it))));
    sec.append(grid);
    return sec;
  }
  ...
  async function buildItemMap(t) {
    if (window.CSRPGems && window.CSRPGems.loadIdMap) await window.CSRPGems.loadIdMap().catch(() => null);
  ...
  async function render() {
    bindBack();
    if (!tradeId) {
      $("#tv-body").innerHTML = '<div class="empty">No trade selected.</div>';
      return;
    }
    ...
    if (!t) {
      $("#tv-body").innerHTML = '<div class="empty">Could not find this trade. It may have expired.</div>';
      return;
    }
    ...
    $("#tv-sub").innerHTML = `<span class="tv-status st-${status}">${STATUS_LABEL[status] || status}</span> · ` + `Created ${esc(fmtDate(t.created_at))}` + (t.completed_at ? ` · Completed ${esc(fmtDate(t.completed_at))}` : t.expires_at ? ` · Expires ${esc(fmtDate(t.expires_at))}` : "");
    const body = $("#tv-body");
    ...
    if (p.known) {
      cols.append(sideBlock("YOU GIVE", "tag-give", p.give.items, p.give.coins, itemMap), sideBlock("YOU RECEIVE", "tag-get", p.get.items, p.get.coins, itemMap));
    } else {
      cols.append(sideBlock(`${esc(t.initiator_name)} GIVES`, "tag-give", p.ini.items, p.ini.coins, itemMap), sideBlock(`${esc(t.recipient_name)} GIVES`, "tag-get", p.rec.items, p.rec.coins, itemMap));
    }
    body.append(cols);
  ...
  function buildActions(t) {
    ...
    bar.innerHTML = `<div class="tv-act-msg" id="tv-act-msg" hidden></div>` + `<div class="tv-act-btns">` + `<button class="btn btn-ghost tv-reject" id="tv-reject">Reject</button>` + `<button class="btn btn-primary tv-accept" id="tv-accept">Accept trade</button>` + `</div>`;
    ...
    async function act(kind) {
      ...
      snd("click");
      const resp = await api(`/api/trades/${encodeURIComponent(t.id)}/${kind}`, {
        method: "PATCH"
      });
      if (resp.ok) {
        ...
        if (stEl) {
          stEl.className = `tv-status st-${s}`;
          stEl.textContent = STATUS_LABEL[s];
        }
      } else {
        busy = false;
