(() => {
  ...
  function makeButton() {
    ...
    btn.id = BTN_ID;
    btn.type = "button";
    btn.className = "rounded-lg bg-theme-primary px-6 py-2 text-sm font-medium text-theme-white transition-colors hover:bg-theme-primary-light";
  ...
  function tickFriendsPage() {
    ...
    for (const img of document.querySelectorAll("img")) {
      ...
      for (let up = 0; row && up < 6; up++, row = row.parentElement) {
        if (row.querySelector("button, svg, a")) {
          hit = row;
          break;
        }
      }
