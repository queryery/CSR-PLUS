(() => {
  ...
  function clickOnce(btn, label) {
    ...
    btn.click();
    setTimeout(() => clicked.delete(btn), 3e3);
  }
  function findPrimaryCTA(text) {
    for (const btn of document.querySelectorAll("button")) {
      if (btn.textContent.trim() !== text) continue;
      if (btn.classList.contains("rounded-full") && btn.classList.contains("bg-theme-primary") && btn.classList.contains("px-12") && visible(btn)) return btn;
    }
    return null;
  }
  function inviteToasts() {
    ...
    for (const btn of document.querySelectorAll("button")) {
      ...
      if (!(btn.classList.contains("bg-theme-primary") && btn.classList.contains("px-4") && btn.classList.contains("hover:bg-red-900"))) continue;
    ...
    if (cfg.autoMatch) {
      ...
      if (matchBtn) {
        if (cfg.acceptInstant) {
          ...
          if (mo && mo.countdownActive()) {} else if (mo && mo.alreadyAccepted()) {} else if (mo && mo.countdownCancelled()) {} else if (mo) {
