(() => {
  ...
  const h = (tag, props = {}, children = []) => {
    ...
    for (const k in props) {
      ...
      if (k === "class") el.className = props[k]; else if (k === "html") el.innerHTML = props[k]; else if (k.startsWith("on") && typeof props[k] === "function") el.addEventListener(k.slice(2).toLowerCase(), props[k]); else if (k === "style" && typeof props[k] === "object") Object.assign(el.style, props[k]); else el.setAttribute(k, props[k]);
    }
  ...
  function findCards() {
    return Array.from(document.querySelectorAll("div.flex.flex-row.border-b.border-theme-gray.rounded-md")).filter(c => c.querySelector("h1") && c.querySelector("img[alt]"));
  }
  function findTeamColumns() {
    const cols = Array.from(document.querySelectorAll("div.flex.flex-col.justify-start.w-full.space-y-1\\.5"));
    return cols.map(col => ({
      el: col,
      title: col.querySelector("p.font-semibold")?.textContent.trim() || "",
      cards: Array.from(col.querySelectorAll("div.flex.flex-row.border-b.border-theme-gray.rounded-md")).filter(c => c.querySelector("h1")).map(parseCard)
    }));
  }
  function findMatchFoundModal() {
    ...
    if (!host) return null;
    const avatars = Array.from(host.querySelectorAll('img[alt="Avatar"], img[alt]')).filter(im => {
    ...
    const accepted = !!acceptBtn && (acceptBtn.disabled || acceptBtn.textContent.trim().toLowerCase() === "match accepted");
    return {
      host,
      inner,
      avatars,
