(() => {
  ...
  function friendCard(f) {
    ...
    btn.innerHTML = (avatar ? `<img class="fr-av" src="${avatar}" alt="" />` : '<div class="fr-av"></div>') + `<div class="fr-info"><div class="fr-name">${esc(name)}</div><div class="fr-sub">#${esc(id.slice(-6))}</div></div>`;
  ...
  function card(it, side) {
    ...
    const floatBadge = w ? `<span class="c-float" style="color:${w.c};border-color:${w.c}55"><span class="c-dot" style="background:${w.c}"></span>${w.code} · ${Number(it.float).toFixed(4)}</span>` : "";
  ...
  function visibleItems(side) {
    ...
    list = list.slice();
    if (s.sort === "rarity") list.sort((a, b) => b.rarity - a.rarity || String(a.name).localeCompare(String(b.name))); else if (s.sort === "float") list.sort((a, b) => (a.float ?? 99) - (b.float ?? 99)); else list.sort((a, b) => String(a.name).localeCompare(String(b.name)));
    return list;
  }
  const PAGE = 50;
  ...
  function renderSide(side) {
    ...
    const s = state[side];
    if (window.CSRPGems && window.CSRPGems.learn) window.CSRPGems.learn(s.items);
    if (scrollHandlers[side]) {
      gridEl.removeEventListener("scroll", scrollHandlers[side]);
      scrollHandlers[side] = null;
    }
    gridEl.innerHTML = "";
    gridEl.scrollTop = 0;
  ...
  function buildCats(side) {
    ...
    Object.keys(counts).sort((a, b) => counts[b] - counts[a]).forEach(t => box.append(mk(String(t), typeName(Number(t)), counts[t])));
  }
  function skeletons(side) {
    $(gridSel(side)).innerHTML = Array.from({
      length: 12
  ...
  const itemsOf = data => Array.isArray(data) ? data : Array.isArray(data?.items) ? data.items : Array.isArray(data?.inventory) ? data.inventory : [];
  ...
  function paintIfActive(side) {
    buildCats(side);
    if (state.activeSide === side) renderSide(side); else state[side].dirty = true;
    applyCoinCaps();
  }
  let mePromise = null;
  function getMe() {
    ...
    mePromise = (async () => {
      ...
      const stashed = await new Promise(res => {
        try {
          chrome.storage.local.get([ "csrpMyId" ], d => res(d && d.csrpMyId ? String(d.csrpMyId) : null));
        } catch {
          res(null);
        }
      });
  ...
  function bindCoinInput(input, side) {
    ...
    input.addEventListener("input", () => {
      guard();
      clampCoin(input, state[side].coins);
      updateSummary();
    });
  ...
  function offer() {
    return {
      give: {
        items: [ ...state.me.sel ],
        coins: readCoins($("#me-coins"))
      },
      get: {
        items: [ ...state.them.sel ],
        coins: readCoins($("#them-coins"))
      }
    };
  }
  ...
  function switchSide(side) {
    ...
    if (state[side].loaded && state[side].dirty) {
      state[side].dirty = false;
      renderSide(side);
    } else if (side === "them" && state.partner && !state.themLoaded) loadTheirInventory(state.partner.id); else {
      const g = document.querySelector(gridSel(side));
      if (g) g.scrollTop = 0;
    }
  }
  ...
  function overlayResult(ok, info) {
    ...
    $("#to-sub").textContent = ok ? `Your offer to ${state.partner.name} is now pending their response.` : info.error || "The server rejected this offer.";
  ...
  async function send() {
    ...
    const idList = (arr, owned) => "[" + arr.map(String).filter(x => owned.has(x) && /^\d+$/.test(x)).join(",") + "]";
    const body = `{"recipient_id":"${String(state.partner.id).replace(/[^\d]/g, "")}",` + `"initiator_items":${idList(o.give.items, ownIds)},` + `"initiator_coins":${o.give.coins},` + `"recipient_items":${idList(o.get.items, theirIds)},` + `"recipient_coins":${o.get.coins},` + `"client_ref":"${ref}"}`;
    ...
    if (resp.ok) {
      ...
      $("#them-coins").value = "0";
      renderSide("me");
      if (state.themLoaded) renderSide("them");
      updateTabBadges();
      updateSummary();
    } else {
      const apiMsg = resp.data && (resp.data.message || resp.data.error || (typeof resp.data === "string" ? resp.data : ""));
      overlayResult(false, {
        error: apiMsg || resp.error || "Request failed (HTTP " + (resp.status || "?") + ")",
        ref
      });
      updateSummary();
    }
  ...
  function perspective(t, myId) {
    ...
    const ini = {
      items: t.items_from_initiator || [],
      coins: t.coins?.initiator_coins || 0
    };
    const rec = {
      items: t.items_from_recipient || [],
      coins: t.coins?.recipient_coins || 0
    };
  ...
  function tradeRow(t, myId) {
    ...
    head.innerHTML = `<div class="tr-who"><div class="tr-who-name">` + `<span class="${String(t.initiator_id) === String(myId) ? "tr-me" : ""}">${esc(sender)}</span>` + `<span class="tr-arrow">→</span>` + `<span class="${String(t.recipient_id) === String(myId) ? "tr-me" : ""}">${esc(receiver)}</span>` + `</div>` + `<div class="tr-who-sub">give ${esc(sideSummary(p.give))} · get ${esc(sideSummary(p.get))}</div></div>` + `<span class="tr-status">${STATUS_LABEL[status] || status || "—"}</span>` + `<span class="tr-caret tr-open-ic" title="Open trade">↗</span>`;
    card.append(head);
    head.addEventListener("click", () => {
      snd("click");
      const url = chrome.runtime.getURL(`trades/trade-view.html?id=${encodeURIComponent(t.id)}${myId != null ? "&me=" + encodeURIComponent(myId) : ""}`);
  ...
  function previewItem(it) {
    ...
    row.innerHTML = `<span class="trp-strip"></span>` + (urls.length ? `<img class="trp-img" alt="" />` : '<span class="trp-img"></span>') + `<span class="trp-meta"><span class="trp-name" style="color:${rar.c}">${star ? "★ " : ""}${esc(weapon)}</span>` + `<span class="trp-skin">${esc(skin || rar.name)}</span></span>` + `<span class="trp-tags">${st}${wearBadge}</span>`;
  ...
  function previewColumn(label, cls, side) {
    ...
    if (!side.items.length && !side.coins) wrap.append(el("div", "trp-empty", "Nothing")); else {
  ...
  function showPreview(t, p) {
    ...
    box.innerHTML = `<div class="trp-head"><span class="trp-title">${esc(t.initiator_name)} → ${esc(t.recipient_name)}</span>` + `<span class="trp-status">${STATUS_LABEL[status] || status}</span></div>`;
  ...
  function statusMatch(t) {
    ...
    switch (tradeState.filter) {
     ...
     case "rejected":
      return status === "rejected" || status === "cancelled" || status === "canceled" || status === "expired";
  ...
  function renderTrades() {
    ...
    box.innerHTML = "";
    if (!list.length) {
      box.append(el("div", "empty", tradeState.list.length ? "No trades match your filters." : "No trades yet."));
      return;
    }
  ...
  async function loadTrades(force) {
    ...
    if (!resp.ok) {
      $("#trade-list").innerHTML = '<div class="empty">Could not load your trades. Make sure you\'re signed in on csrestored.fun.</div>';
      return;
    }
    renderTrades();
  }
  ...
  function bindTradesList() {
    ...
    $("#tl-search").addEventListener("input", e => {
      tradeState.q = e.target.value.trim().toLowerCase();
      search();
    });
    $("#tl-from").addEventListener("change", e => {
      tradeState.from = e.target.value;
      renderTrades();
    });
    $("#tl-to").addEventListener("change", e => {
      tradeState.to = e.target.value;
      renderTrades();
    });
  ...
  function bindControls() {
    ...
    const meSearch = debounce(() => {
      renderSide("me");
    }, 130);
    $("#me-search").addEventListener("input", e => {
      state.me.q = e.target.value.trim().toLowerCase();
      meSearch();
    });
    const themSearch = debounce(() => {
      renderSide("them");
    }, 130);
    $("#them-search").addEventListener("input", e => {
      state.them.q = e.target.value.trim().toLowerCase();
      themSearch();
    });
    bindSort("me");
    bindSort("them");
    ...
    $("#to-close").addEventListener("click", () => {
      snd("click");
      hideOverlay();
      backToFriends();
    });
    ...
    const openReport = () => {
      snd("alert");
      reportOv.hidden = false;
    };
    const closeReport = () => {
      snd("off");
      reportOv.hidden = true;
    };
    $("#report-bug").addEventListener("click", openReport);
    $("#report-x").addEventListener("click", closeReport);
    reportOv.addEventListener("click", e => {
      if (e.target === reportOv) closeReport();
    });
    document.addEventListener("keydown", e => {
      if (e.key === "Escape" && !reportOv.hidden) closeReport();
    });
  ...
  function init() {
    ...
    loadTrades(false);
    if (window.CSRPGems && window.CSRPGems.loadIdMap) {
      window.CSRPGems.loadIdMap().then(() => {
        try {
          if (state.me.items.length) renderSide("me");
          if (state.them.items.length) renderSide("them");
        } catch {}
      }).catch(() => {});
    }
  }
