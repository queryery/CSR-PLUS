(() => {
  ...
  function sellPrice(r, f, st) {
    ...
    let p = Math.round((RARITY_PRICES[Number(r)] || 0) * (1 - fl * .25));
  ...
  function getSpecialsMap() {
    ...
    specialsPromise = (async () => {
      ...
      try {
        ...
        try {
          localStorage.setItem(SPECIALS_KEY, JSON.stringify({
            t: Date.now(),
            v: specialsMemo
          }));
        } catch {}
  ...
  function visibleCases() {
    ...
    if (state.kind === "special") list = list.filter(c => hasSpecial(c.id)); else if (state.kind === "cases") list = list.filter(c => !isCapsule(c)); else if (state.kind === "agents") list = list.filter(isAgents); else if (state.kind === "other") list = list.filter(c => isCapsule(c) && !isAgents(c));
    if (state.sort === "price") list.sort((a, b) => a.price - b.price || a.name.localeCompare(b.name)); else if (state.sort === "price-d") list.sort((a, b) => b.price - a.price || a.name.localeCompare(b.name)); else list.sort((a, b) => a.name.localeCompare(b.name));
    return list;
  }
  function caseCard(c, i) {
    const special = hasSpecial(c.id);
    const d = el("div", "case-card" + (special ? " has-special" : ""));
    d.style.animationDelay = Math.min(i * .018, .45) + "s";
    d.innerHTML = (special ? '<span class="cc-star" title="Can drop a rare special item">★</span>' : "") + `<div class="cc-art"><img src="${caseArtUrl(c.id)}" alt="" loading="lazy" decoding="async" /></div>` + `<div class="cc-name">${esc(c.name)}</div>` + `<div class="cc-price">${coin(c.price)}</div>`;
    const im = d.querySelector("img");
    if (im) im.addEventListener("error", () => im.replaceWith(el("div", "cc-fallback", "▣")));
    d.addEventListener("mouseenter", () => snd("hover", .4));
  ...
  function renderList() {
    ...
    grid.innerHTML = "";
    if (!list.length) {
      grid.append(el("div", "empty", "No cases match."));
      return;
    }
  ...
  function loaderEl(label) {
    const d = el("div", "cs-loading", `<span class="cs-spin" aria-hidden="true"></span>\n       <div class="cs-load-txt"><b>${esc(label)}</b><span class="cs-load-sub">Contacting the CS:R servers…</span></div>`);
    const sub = d.querySelector(".cs-load-sub");
    setTimeout(() => {
      if (d.isConnected) sub.textContent = "Still loading — the CS:R servers are slow right now, hang tight…";
    }, 4e3);
    setTimeout(() => {
      if (d.isConnected) sub.textContent = "The servers are really struggling… we're still trying.";
    }, 12e3);
    return d;
  }
  function errorEl(title, detail, onRetry) {
    const d = el("div", "cs-error", `<div class="cs-error-txt"><b>${esc(title)}</b><span>${esc(detail)}</span></div>`);
  ...
  async function loadCases() {
    ...
    if (!state.cases.length) {
      ...
      grid.append(timedOut ? errorEl("The CS:R servers didn't respond", "They're slow or down right now — nothing wrong on your end.", loadCases) : errorEl("Could not load cases", "Make sure you're signed in on csrestored.fun, then try again.", loadCases));
      return;
    }
    renderList();
  }
  ...
  async function openCaseView(id) {
    ...
    if (!resp.ok || !resp.data || !resp.data.case) {
      const timedOut = resp && (resp.timeout || /timeout/i.test(resp.error || ""));
      $("#case-hero").innerHTML = "";
      $("#case-hero").append(timedOut ? errorEl("The CS:R servers didn't respond", "They're slow or down right now — try again in a moment.", () => openCaseView(id)) : errorEl("Could not load this case", "Something went wrong fetching it — try again.", () => openCaseView(id)));
      $("#contents").innerHTML = "";
      return;
    }
  ...
  function renderHero() {
    ...
    info.append(el("div", "ch-name", esc(c.name)), el("div", "ch-sub", `${state.current.items.length} items inside` + (hasSpecial(c.id) ? ' · <span style="color:var(--gold)">★ rare special possible</span>' : "")));
    ...
    quickBtn.addEventListener("click", () => spin({
      quick: true
    }));
    ...
    buyBtn.addEventListener("click", async () => {
      ...
      const resp = await api(`/inventory/cases/buy/${encodeURIComponent(c.id)}`, {
        method: "POST",
        body: ""
      });
  ...
  function renderContents() {
    const grid = $("#contents");
    grid.innerHTML = "";
    const items = state.current.items.slice().sort((a, b) => Number(b.rarity) - Number(a.rarity) || String(a.name).localeCompare(String(b.name)));
    const frag = document.createDocumentFragment();
  ...
  function itemCard(it, i) {
    ...
    c.style.animationDelay = Math.min(i * .02, .4) + "s";
    ...
    c.querySelector(".c-img").addEventListener("error", function() {
      this.remove();
  ...
  async function appendSpecialCards(token) {
    ...
    if (list && list.length) {
      ...
      frag.append(specialCard({
        n: "Rare Special Item",
        s: "Pool unknown — unbox to find out",
        p: null,
        i: null
      }, 0));
    }
  ...
  function showMsg(text, ok) {
    const m = $("#case-msg");
    m.hidden = false;
    m.className = "case-msg" + (ok ? " ok" : "");
    m.textContent = text;
  }
  function hideMsg() {
    $("#case-msg").hidden = true;
  }
  ...
  function reelCardData(c) {
    ...
    for (const it of state.current.items) {
      ...
      for (let i = 0; i < w; i++) pool.push({
        id: it.id,
        name: it.name,
        rarity: it.rarity
      });
    }
    if (hasSpecial(c.id)) pool.push({
      special: true,
  ...
  function reelItemEl(d) {
    ...
    r.innerHTML = d.special ? `<div class="ri-img" style="display:flex;align-items:center;justify-content:center;font-size:44px;color:var(--gold)">★</div><span class="ri-name">Rare Special</span><span class="ri-skin">???</span>` : `<img class="ri-img" alt="" loading="lazy" decoding="async" src="${iconUrl(d.id)}" />` + `<span class="ri-name">${esc(weapon)}</span><span class="ri-skin">${esc(skin)}</span>`;
  ...
  function animateReel(landIdx, skip, duration = 6800) {
    ...
    return new Promise(resolve => {
      ...
      const jitter = (Math.random() - .5) * (ITEM_W * .55);
  ...
  async function spin({quick = false} = {}) {
    ...
    const resp = await api(`/inventory/cases/open/${encodeURIComponent(c.id)}`, {
      method: "POST",
      body: ""
    });
    ...
    hideMsg();
    const d = resp.data;
    const drop = d.item && typeof d.item === "object" && d.item || d.dropped_item && typeof d.dropped_item === "object" && d.dropped_item || d.droppedItem && typeof d.droppedItem === "object" && d.droppedItem || d;
    if (state.coins != null) {
      state.coins -= c.price || 0;
      renderBalance();
    }
    let landCard;
    ...
    if (!quick) {
      ...
      await skippableWait(skip.on ? 250 : 900, {
        on: false,
        cbs: skip.cbs
      });
      activeSkip = null;
  ...
  function showReveal(drop, landCard) {
    ...
    card.classList.remove("rv-anim");
    void card.offsetWidth;
    card.classList.add("rv-anim");
    const art = $("#rv-art");
    ...
    const gem = window.CSRPGems ? window.CSRPGems.badgeFor({
      name: fullName,
      seed,
      skin_index: drop?.skin_index,
    ...
    if (drop && weaponId) {
      ...
      sell.onclick = async () => {
        ...
        const resp = await api(`/inventory/sell/${encodeURIComponent(weaponId)}`, {
          method: "POST",
          body: ""
        });
        if (resp.ok) {
  ...
  function init() {
    ...
    $("#rv-close").addEventListener("click", () => {
      snd("off");
      hideReveal();
    });
    $("#rv-x").addEventListener("click", () => {
      snd("off");
      hideReveal();
    });
    $("#rv-again").addEventListener("click", () => {
      snd("click");
      hideReveal();
      spin({
    ...
    document.addEventListener("keydown", e => {
      ...
      if (EMBEDDED && !state.spinning && $("#open-stage").hidden) {
        if (!$("#page-case").hidden) backToList(); else closeOverlay();
      }
    });
    loadCases();
    loadBalance();
    try {
      localStorage.removeItem("csrp:specials-v1");
    } catch {}
    getSpecialsMap().then(() => {
      if (state.q) renderList();
    });
  }
  init();
})();
