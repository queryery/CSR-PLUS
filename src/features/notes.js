(() => {
  ...
  async function openCardPopover(id, name, anchorEl) {
    ...
    }), tier.label ]) ]), agg ? h("div", {
      class: "csrp-pop-stats"
    }, [ h("span", {}, `K/D ${agg.kd.toFixed(2)}`), h("span", {}, `K/R ${agg.kr.toFixed(2)}`), h("span", {}, `ADR ${agg.adr.toFixed(0)}`), h("span", {}, `${(agg.winrate * 100).toFixed(0)}% WR`), h("span", {}, `${agg.games}g`) ]) : null, tagInput, noteArea, h("div", {
      class: "csrp-pop-actions"
    }, [ h("button", {
      class: "csrp-pop-save",
      onclick: () => {
        saveNote(id, {
  ...
  async function openProfilePanel(id, name) {
    ...
    p.innerHTML = "";
    p.append(h("div", {
      class: "csrp-panel-skel"
    }, "Loading " + (name || "player") + "…"));
    const [profile, history, agg] = await Promise.all([ CSRP.api.user(id), CSRP.api.history(id), CSRP.playerBadges.getAgg(id) ]);
    p.innerHTML = "";
    const tier = CSRP.classify(agg);
    const note = notes[id] || {};
    const head = h("div", {
      class: "csrp-panel-head"
      ...
      class: "csrp-panel-name"
    }, profile?.name || name || "Player"), h("div", {
      class: "csrp-panel-sub"
    }, `${profile?.points ?? "—"} ELO · ${profile?.matches ?? 0} matches`), h("span", {
      class: `csrp-badge ${tier.cls}`
    }, tier.label) ]), h("button", {
      class: "csrp-panel-x",
      onclick: close,
    ...
    const statGrid = agg ? h("div", {
      class: "csrp-panel-stats"
    }, [ [ "K/D", agg.kd.toFixed(2) ], [ "K/R", agg.kr.toFixed(2) ], [ "ADR", agg.adr.toFixed(0) ], [ "Winrate", (agg.winrate * 100).toFixed(0) + "%" ], [ "Form", agg.recentForm.toFixed(2) + "×" ], [ "Sample", agg.games + "g" ] ].map(([k, v]) => h("div", {
      class: "csrp-panel-stat"
    }, [ h("div", {
      class: "csrp-panel-statv"
    }, v), h("div", {
      class: "csrp-panel-statk"
    }, k) ]))) : null;
    const tagInput = h("input", {
      class: "csrp-note-tag",
    ...
    noteArea.value = note.text || "";
    const notesBlock = h("div", {
      class: "csrp-panel-notes"
    }, [ h("div", {
      class: "csrp-panel-h"
    }, "Notes & tags"), tagInput, noteArea, h("button", {
    ...
    const histBlock = h("div", {
      class: "csrp-panel-hist"
    }, [ h("div", {
      class: "csrp-panel-h"
    }, "Recent matches"), ...recent.length ? recent.map(m => {
      ...
      return h("div", {
        class: "csrp-hist-row " + (won ? "csrp-w" : "csrp-l")
      }, [ h("span", {
        class: "csrp-hist-map"
      }, (m.map || "").replace("de_", "")), h("span", {
        class: "csrp-hist-kda"
      }, `${pl.kills}/${pl.deaths}/${pl.assists}`), h("span", {
        class: "csrp-hist-score"
      }, m.score), h("span", {
        class: "csrp-hist-res"
      }, won ? "W" : "L") ]);
    }) : [ h("div", {
      class: "csrp-panel-empty"
    }, "No recent matches") ] ]);
    p.append(head, statGrid, notesBlock, histBlock, h("a", {
  ...
  function tick() {
    for (const card of CSRP.dom.findCards()) {
      ...
      if (note?.tag) {
        if (existing) existing.textContent = note.tag; else head.appendChild(h("span", {
          class: "csrp-tag-chip"
        }, note.tag));
