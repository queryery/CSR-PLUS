(() => {
  ...
  function dataRows(table) {
    return Array.from(table.querySelectorAll("div.grid.grid-cols-5.items-center")).filter(r => !r.classList.contains("csrp-lb-remote"));
  }
  ...
  function buildRemoteRow(id, profile, state) {
    const avatar = profile && profile.avatar ? `https://cdn.discordapp.com/avatars/${id}/${profile.avatar}.png?size=64` : "";
    ...
    return h("div", {
      ...
      class: "csrp-lb-remote-tag"
    }, state === "loading" ? "⌕" : "★")), h("a", {
      class: "flex flex-row items-center space-x-3",
      href: `${SITE}/app/user/${id}`
    }, [ av, h("p", {
      class: "truncate text-left text-theme-white"
    }, state === "loading" ? "Looking up…" : name) ]), h("p", {}, matches), h("p", {}, wins), h("p", {
  ...
  function showRemote(table, header, id, emptyEl) {
    ...
    if (remoteCache.has(id)) {
      ...
      if (p) place(buildRemoteRow(id, p, "ok")); else notFound();
      return;
    }
    ...
    Promise.resolve(CSRP.api.user(id)).then(profile => {
      ...
      if (ok) place(buildRemoteRow(id, profile, "ok")); else notFound();
    }).catch(() => {
  ...
  function applyFilter(table, header, counterEl, emptyEl) {
    ...
    if (counterEl) {
      counterEl.textContent = q ? `${shown + remoteShown} / ${rows.length}` : `${rows.length} players`;
    }
  }
  function build(header) {
    ...
    const bar = h("div", {
      id: BOX_ID,
      class: "csrp-lb-search"
    }, [ h("span", {
      class: "csrp-lb-ic"
    }, "⌕"), input, clearBtn, counter ]);
  ...
  function tick() {
    ...
    if (!document.getElementById(BOX_ID)) build(header); else {
      const table = findTable(header);
      const bar = document.getElementById(BOX_ID);
      applyFilter(table, header, bar?.querySelector(".csrp-lb-count"), document.querySelector(".csrp-lb-empty"));
    }
  }
  CSRP.leaderboardSearch = {
    tick
  };
})();
