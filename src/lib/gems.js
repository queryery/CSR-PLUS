...
(function (root) {
  ...
  const PHASE_COLORS = {
    'Ruby': '#ef2d4f', 'Sapphire': '#2f7bff', 'Black Pearl': '#8a63d2', 'Emerald': '#16c784',
    'Phase 1': '#b78bff', 'Phase 2': '#e07bd0', 'Phase 3': '#9b8bd0', 'Phase 4': '#7f9bff',
  };
  const GEM_PHASES = ['Ruby', 'Sapphire', 'Black Pearl', 'Emerald'];
  const DOPPLER = { 415: 'Ruby', 416: 'Sapphire', 417: 'Black Pearl', 418: 'Phase 1', 419: 'Phase 2', 420: 'Phase 3', 421: 'Phase 4' };
  const GAMMA = { 568: 'Emerald', 569: 'Phase 1', 570: 'Phase 2', 571: 'Phase 3', 572: 'Phase 4' };
  const GAMMA_GLOCK = { 1119: 'Emerald', 1120: 'Phase 1', 1121: 'Phase 2', 1122: 'Phase 3', 1123: 'Phase 4' };
  ...
  function persistMap() {
    ...
    persistTimer = setTimeout(() => {
      try { localStorage.setItem(MAP_CACHE_KEY, JSON.stringify({ t: Date.now(), v: idMap })); } catch {  }
    }, 400);
  }
  ...
  function loadIdMap() {
    ...
    mapPromise = (async () => {
      ...
      try {
        ...
        if (c && c.v) { mergeMap(c.v); fresh = Date.now() - c.t < MAP_TTL; }
      } catch {  }
      if (!fresh) {
        try {
  ...
  function learn(items) {
    ...
    for (const it of items) {
      ...
      if (idMap[String(id)] !== fc) { idMap[String(id)] = fc; changed = true; }
    }
  ...
  function phaseWord(s) {
    ...
    const map = { sapphire: 'Sapphire', ruby: 'Ruby', 'black pearl': 'Black Pearl', emerald: 'Emerald' };
    return map[w] || null;
  }
  ...
  const CH_WEAPON_RULES = [
    ['ak47', /ak-47/], ['mac10', /mac-10/], ['fiveseven', /five-seven/],
    ['m9', /m9 bayonet/], ['karambit', /karambit/], ['butterfly', /butterfly/],
    ['skeleton', /skeleton/], ['talon', /talon/], ['stiletto', /stiletto/],
    ['ursus', /ursus/], ['nomad', /nomad/], ['paracord', /paracord/],
    ['survival', /survival/], ['navaja', /navaja/], ['falchion', /falchion/],
    ['huntsman', /huntsman/], ['bowie', /bowie/], ['flip', /flip knife/],
    ['gut', /gut knife/], ['classic', /classic knife/], ['kukri', /kukri/],
    ['shadow', /shadow daggers/], ['bayonet', /bayonet/],
  ];
  ...
  const CH_BLUE = {
    ak47: {
      ...
      t3: [34, 81, 112, 278, 310, 312, 363, 381, 413, 428, 429, 450, 519, 557, 586, 610, 647, 685, 689, 690, 733, 754, 770, 819, 823, 856, 862, 872, 878, 935, 1000],
    },
    karambit: {
      1: [387],
    ...
    m9: { 1: [601], t1: [58, 107, 150, 239, 253, 349, 354, 403, 406, 417, 449, 503, 517, 523, 550, 585, 634, 675, 897, 946] },
    fiveseven: { 1: [278, 690], t1: [189, 363, 689, 868, 872] },
    mac10: { t1: [667, 114, 406, 95], t2: [19, 22, 80, 199, 239, 251, 295, 313, 349, 503, 748, 807, 890, 944] },
    butterfly: { 1: [494] },
    bayonet: { 1: [555, 592, 670] },
    talon: { 1: [146, 442] },
    skeleton: { 1: [387, 601] },
    flip: { 1: [670] },
    bowie: { 1: [182] },
  ...
  const CH_GOLD = { flip: { 1: [731] } };

  const CH_BADGES = {
    blue: {
      1: { label: '◆ #1 BLUE GEM', color: '#1f8fff' },
      t1: { label: '◆ T1 Blue Gem', color: '#2f7bff' },
      t2: { label: '◆ T2 Blue Gem', color: '#4a8fe0' },
      t3: { label: '◆ T3 Blue', color: '#6aa6d8' },
    },
    gold: {
      1: { label: '◆ #1 GOLD GEM', color: '#e4ae39' },
      t1: { label: '◆ G1 Gold', color: '#e4ae39' },
      t2: { label: '◆ G2 Gold', color: '#d8b35c' },
      t3: { label: '◆ G3 Gold', color: '#c9b078' },
    },
  };
  ...
  function gemFor(name, seed) {
    ...
    if (tier == null) { kind = 'gold'; tier = lookupTier(CH_GOLD[key], s); }
    if (tier == null) return null;
    const b = CH_BADGES[kind][tier];
    return { ...b, gem: tier === 1, title: `Case Hardened seed #${s} — ${b.label.replace('◆ ', '')} (community pattern lists)` };
  }
  ...
  function badgeFor(item) {
    ...
    let skinIndex = [item.skin_index, item.skinIndex, item.finish_catalog, item.paint_index, item.paintIndex, item.finish_id, item.phase, item.skin_phase]
      .find((v) => v != null);
  ...
  const api = { badgeFor, phaseFor, gemFor, loadIdMap, learn, defId };
  root.CSRPGems = api;
