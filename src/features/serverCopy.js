(() => {
  ...
  function looksReady(text) {
    ...
    return /connect\s+\d{1,3}(\.\d{1,3}){3}:\d+/i.test(text) || /\d{1,3}(\.\d{1,3}){3}:\d+/.test(text) || /steam:\/\/connect\//i.test(text);
  }
  function toast(msg) {
    ...
    setTimeout(() => {
      t.classList.remove("csrp-toast-show");
