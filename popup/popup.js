(() => {
  ...
  function saveDebounced(key, value, ms = 250) {
    ...
    saveTimers[key] = setTimeout(() => SA.set({
      [key]: value
    }), ms);
  }
  ...
  async function loadFriends() {
    ...
    box.innerHTML = Array.from({
      length: 5
    }, () => '<div class="friend-skel"></div>').join("");
    try {
      const res = await fetch("https://api.csrestored.fun/users/friends", {
        credentials: "include"
      });
      ...
      friends.forEach(f => {
        ...
        row.querySelector(".ftrade").addEventListener("click", e => {
          ...
          snd("click");
          chrome.tabs.create({
            url: chrome.runtime.getURL(`trades/trades.html?partner=${f.id}`)
          });
        });
        ...
        row.addEventListener("click", () => {
          ...
          if (now) state.inviteFriends[f.id] = true; else delete state.inviteFriends[f.id];
  ...
  function bindDrag(ul) {
    ...
    ul.addEventListener("dragover", e => {
      ...
      if (after == null) ul.appendChild(dragEl); else if (after !== dragEl) ul.insertBefore(dragEl, after);
    });
  }
  function afterEl(ul, y) {
    ...
    for (const el of $$(".map:not(.drag)", ul)) {
      ...
      if (offset < 0 && offset > closest.offset) closest = {
        offset,
        el
      };
    }
  ...
  async function renderChangelog() {
    ...
    try {
      ...
      const items = head.filter(l => l.trim().startsWith("- ")).map(l => l.replace(/^[-\s]+/, "").replace(/\*\*/g, "").trim()).slice(0, 6);
      box.innerHTML = `<div class="cl-ver">${esc(title)}</div>` + '<ul class="cl-list">' + items.map(i => `<li>${esc(i)}</li>`).join("") + "</ul>" + `<a class="cl-more" href="https://github.com/${REPO}/blob/main/CHANGELOG.md" target="_blank" rel="noopener">Full changelog ↗</a>`;
    } catch {
      box.remove();
    }
  }
  ...
  async function enforceMinVersion() {
    ...
    if (body) body.innerHTML = esc(cfg.updateMessage || "This version of CSR+ is no longer supported. Update to keep using it.") + `<br><b>Minimum required: v${esc(cfg.minVersion)}</b>`;
  ...
  function bindStudio() {
    $("#open-studio")?.addEventListener("click", () => {
      snd("click");
      chrome.tabs.create({
        url: chrome.runtime.getURL("customize/customize.html")
      });
    });
    $("#donate-btn")?.addEventListener("click", () => {
      snd("click");
      chrome.tabs.create({
        url: "https://www.paypal.com/paypalme/VYavorskiy"
      });
    });
    bindProAccount();
    bindReports();
  ...
  const REP_STATUS = {
    sent: {
      label: "Sent",
      cls: "s-sent",
      desc: "Delivered to moderators"
    },
    checking: {
      label: "Checking",
      cls: "s-checking",
      desc: "A moderator is reviewing it"
    },
    punished: {
      label: "Punishment applied",
      cls: "s-punished",
      desc: "Action was taken against the player"
    },
    insufficient: {
      label: "Not enough info",
      cls: "s-insufficient",
      desc: "Add more detail and report again"
    },
    rejected: {
      label: "Rejected",
      cls: "s-rejected",
      desc: "Dismissed as invalid or fake"
    }
  };
  let repLoaded = false;
  ...
  async function loadReports() {
    ...
    const resp = await proProxy("/me/reports", {
      token: proSession.token
    });
    repLoaded = true;
    if (resp.ok && resp.data && Array.isArray(resp.data.reports)) renderReports(resp.data.reports); else if (resp.status === 401) {
      renderReports([]);
  ...
  function bindReports() {
    $("#rep-signin")?.addEventListener("click", async () => {
      snd("click");
      $("#rep-signin").disabled = true;
      const s = await proSignIn();
      $("#rep-signin").disabled = false;
      if (!s) return toast("Sign-in failed or cancelled");
      await refreshPro();
      await loadReports();
    });
    $("#rep-refresh")?.addEventListener("click", () => {
      snd("click");
      repLoaded = false;
      loadReports();
    });
  ...
  const proValid = s => !!(s && s.token && s.exp && s.exp * 1e3 > Date.now() + 3e4);
  function loadPro() {
    return new Promise(r => chrome.storage.local.get([ JWT_KEY ], d => {
  ...
  async function proSignIn() {
    ...
    if (!code) return null;
    const resp = await proProxy("/auth/exchange", {
      method: "POST",
      body: {
        code,
        redirectUri
  ...
  function reflectPro() {
    const signedIn = proValid(proSession);
    const st = $("#pro-status");
    if (st) st.textContent = signedIn ? `${proSession.user?.name || "Signed in"} · sharing enabled` : "Not signed in";
    ...
    area.get([ "hideBanners" ], d => {
      $("#pop-hide-banners")?.classList.toggle("on", d.hideBanners === true);
    });
  }
  async function refreshPro() {
    if (!proValid(proSession)) {
      proTier = "free";
      reflectPro();
      return;
    }
    const v = chrome.runtime.getManifest().version;
    const resp = await proProxy(`/me?v=${encodeURIComponent(v)}`, {
      token: proSession.token
    });
  ...
  function bindProAccount() {
    ...
    $("#pro-signin")?.addEventListener("click", async () => {
      snd("click");
      $("#pro-signin").disabled = true;
      const s = await proSignIn();
      $("#pro-signin").disabled = false;
      if (!s) return toast("Sign-in failed or cancelled");
      await refreshPro();
      toast("Signed in");
    });
    ...
    $("#pop-hide-banners")?.addEventListener("click", () => {
      ...
      if (area !== chrome.storage.local) chrome.storage.local.set({
        hideBanners: on
      });
    });
  }
  function bindDocs() {
    const open = e => {
      ...
      snd("click");
      chrome.tabs.create({
        url: chrome.runtime.getURL("docs/docs.html")
      });
    };
