(() => {
  ...
  chrome.storage.onChanged.addListener((c, area) => {
    ...
    if (c[KEY]) { localCfg = c[KEY].newValue || null; sharedCache.clear(); cleanup(); }
  ...
  function pubToCfg(p) {
    ...
    return {
      ...
      nameStyle: p.nameStyle || 'none', cardFlair: p.cardFlair || 'none',
      avatarFrame: p.avatarFrame || 'none',
      chip: p.chip || '', chipStyle: p.chipStyle || 'outline', kanji: p.kanji || '',
      banner: p.banner || null, bannerOn: !!p.banner, bannerKind: p.bannerKind || 'static',
  ...
  function scheduleFetch() {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const now = Date.now();
        for (const id of ids) sharedCache.set(id, { t: now, cfg: pubToCfg(profiles[id]) });
      } catch { for (const id of ids) sharedCache.set(id, { t: Date.now(), cfg: null }); }
    }, 60);
  }
  ...
  function gateLocalCfg(c, tier) {
    if (!c) return c;
    const out = { ...c, tier };
    if (TIER_RANK[tier] < TIER_RANK.pro) { out.banner = null; out.bannerOn = false; }
    if (tier !== 'premium') {
  ...
  function cfgForId(id) {
    if (myId && id === myId) {
      ...
      if (tier === undefined) { pendingIds.add(id); scheduleFetch(); }
      if (!localCfg) return tier && tier !== 'free' ? { enabled: true, tier, __own: true } : undefined;
      return { ...gateLocalCfg(localCfg, tier || localCfg.tier || 'free'), __own: true };
    }
    const e = sharedCache.get(id);
    if (e && Date.now() - e.t < SHARED_TTL) return e.cfg || undefined;
    pendingIds.add(id); scheduleFetch();
    return undefined;
  }
  ...
  function profileEloAnchor(hdr) {
    ...
    const eloEl = [...hdr.querySelectorAll('div, span, p')]
      .filter((el) => /\bELO\b/i.test(el.textContent) && el.children.length <= 2)
  ...
  function surf(key) {
    ...
    if (s) return { on: !!s.on, x: s.x ?? 50, y: s.y ?? 50, scale: s.scale || 1, rot: s.rot || 0 };
    return {
      on: key === 'profile' ? cfg.bannerOn !== false : !!cfg.cardBanner,
      x: cfg.bannerX ?? 50, y: cfg.bannerY ?? 50, scale: cfg.bannerScale || 1, rot: 0,
    };
  }
  ...
  function layoutBannerImg(layer, img, s) {
    ...
    if (!iw || !ih || !W || !H) return;
    const zoom = clamp(+s.scale || 1, 0.1, 3);
  ...
  function setBannerTextContrast(layer, img, key) {
    ...
    try {
      ...
      const dim = Math.min(0.85, (cfg.bannerDim ?? 30) / 100 + (key === 'profile' ? 0 : 0.3));
  ...
  function applyBanner(layer, key) {
    ...
    if (img && ((wantVideo && img.tagName !== 'VIDEO') || (!wantVideo && img.tagName !== 'IMG'))) {
      if (img._csrpVo) { img._csrpVo.disconnect?.(); }
      img.remove(); img = null;
    }
    if (cfg.fillMode === 'color') {
      ...
      if (!fill) { fill = h('img', { class: 'csrp-pc-bfill', alt: '' }); layer.prepend(fill); }
    ...
    if (!img) {
      ...
      if (wantVideo) {
        img = h('video', { class: 'csrp-pc-bimg', muted: 'true', loop: 'true', playsinline: 'true', preload: 'metadata' });
        img.muted = true;
        img.addEventListener('loadeddata', onReady);
      } else {
        img = h('img', { class: 'csrp-pc-bimg', alt: '' });
    ...
    if (img.dataset.src !== cfg.banner) {
      ...
      if (wantVideo) { img.load?.(); observeVisibility(layer, img); }
      if (fill) fill.src = wantVideo ? '' : cfg.banner;
    } else if (fill && !wantVideo && fill.dataset.src !== cfg.banner) {
      fill.src = cfg.banner; fill.dataset.src = cfg.banner;
    }
  ...
  function ensureCardBanner(card, key) {
    ...
    if (bannerActive(key)) {
      if (!bn) {
        bn = h('div', { class: 'csrp-pc-banner csrp-pc-cardbn', 'aria-hidden': 'true' });
        if (getComputedStyle(card).position === 'static') card.style.position = 'relative';
        card.prepend(bn);
      }
      card.classList.add('csrp-pc-hasbn');
      applyBanner(bn, key);
      card.style.setProperty('--pcdim', Math.min(0.85, (cfg.bannerDim ?? 30) / 100 + 0.3).toFixed(2));
    } else {
      if (bn) bn.remove();
  ...
  function styleCard(card, key) {
    ...
    if (cfg.cardFlair && cfg.cardFlair !== 'none') {
      if (!fx) {
        fx = h('div', { class: 'csrp-pc-fx', 'aria-hidden': 'true' });
        if (getComputedStyle(card).position === 'static') card.style.position = 'relative';
        card.appendChild(fx);
      }
  ...
  function ensureChip(anchor) {
    ...
    if (!chip) {
      chip = h('span', { class: 'csrp-pc-chip' });
    ...
    for (const img of document.querySelectorAll('img[width="76"], img.mr-2')) {
      ...
      withCfg(c, () => {
        ...
        if (bannerActive('profile')) {
          if (!bn) { bn = h('div', { class: 'csrp-pc-banner', 'aria-hidden': 'true' }); hdr.prepend(bn); }
  ...
  function cleanup() {
    ...
    visObservers.clear();
    document.querySelectorAll('.csrp-pc-banner, .csrp-pc-fx, .csrp-pc-overlay, .csrp-pc-chip, .csrp-tier-badge').forEach((n) => { if (n._csrpRo) delete n._csrpRo; if (n._csrpVo) delete n._csrpVo; n.remove(); });
