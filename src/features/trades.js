/* CSR+ — Trades integration on csrestored.fun.
 *
 * Two things happen here:
 *  1. On the native Trades page we drop a "CSR+ Trade" button next to the site's
 *     "Send Trade Offer" button.
 *  2. We intercept clicks on the site's "Trades" nav link and, depending on the
 *     user's setting, either show a one-time prompt ("CSR+ Trades was added"),
 *     send them straight to the CSR+ composer, or leave the legacy page alone.
 *
 * Settings (storage.sync via CSRP.store):
 *   useCsrpTrades         – clicking Trades opens the CSR+ composer.
 *   tradesPromptDismissed – user ticked "Don't ask again"; never prompt again.
 */
(() => {
  'use strict';
  ...
  // Is this anchor the site's "Trades" nav link? It points at the trade-up route
  // (or, defensively, simply reads "Trades").
  ...
  function buildOverlay() {
    ...
    // Persist the dismissal + preferred mode, but only when "don't ask again"
    // is ticked (otherwise this stays a one-off choice).
    ...
    wrap.querySelector('#csrp-tr-legacy').addEventListener('click', async () => {
      ...
      close();
      // Legacy: do nothing — let the site's own navigation stand. If we
      // suppressed the original click, send the user to the legacy page.
      if (wrap.dataset.suppressed === '1') location.assign('/app/inventory/trade-up');
    });
    // Click on the backdrop = same as "legacy" (dismiss, don't redirect).
  ...
  // Decide what happens when the Trades nav link is clicked.
  function handleTradesClick(e) {
    const dismissed = CSRP.store.get('tradesPromptDismissed');
    if (dismissed) {
      // Honour the saved choice silently.
      ...
      return; // legacy: let the site navigate normally
    }
    // Not yet decided → show the prompt and suppress the navigation for now.
    e.preventDefault();
    e.stopPropagation();
    showOverlay(true);
  }

  // Capture-phase click listener so we intercept before the SPA router. Bound
  // once on document; we resolve the Trades link from the event target.
  let navBound = false;
  ...
  // ── the "CSR+ Trade" button on the trades page ────────────────────────────
  function findAnchor() {
