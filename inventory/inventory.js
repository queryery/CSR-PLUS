(() => {
  ...
  function card(it, i) {
    ...
    c.style.animationDelay = Math.min(i * .015, .5) + "s";
    ...
    const img = icon ? `<img class="c-img" src="${icon}" alt="" loading="lazy" onerror="this.closest('.card').classList.add('noimg')" />` : "";
  ...
  function applyView() {
    ...
    if (state.sort === "rarity") list.sort((a, b) => b.rarity - a.rarity || a.name.localeCompare(b.name)); else if (state.sort === "float") list.sort((a, b) => (a.float ?? 99) - (b.float ?? 99)); else if (state.sort === "name") list.sort((a, b) => a.name.localeCompare(b.name));
    const grid = $("#grid");
    grid.innerHTML = "";
    if (!list.length) {
  ...
  function renderStats() {
    ...
    $("#tb-stats").innerHTML = `<span class="kv"><b>${items.length}</b> items</span>` + `<span class="kv"><b>${knives}</b> knives</span>` + `<span class="kv"><b>${covert}</b> covert+</span>` + `<span class="kv"><b>${stat}</b> StatTrak</span>`;
  }
  ...
  async function render() {
    if (embedded) document.querySelector(".topbar")?.remove(); else bindBack();
    if (!id) {
      $("#head").innerHTML = '<div class="ih-skel">No player selected.</div>';
      return;
    }
    ...
    if (profile) {
      ...
      head.innerHTML = (avatar ? `<img class="ih-av" src="${avatar}" alt="" />` : '<div class="ih-av"></div>') + `<div class="ih-info"><div class="ih-name">${esc(profile.name)} ${flag}</div>` + `<div class="ih-sub">Inventory</div></div>`;
    } else {
