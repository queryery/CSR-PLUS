(() => {
  ...
  function jwtExp(t) { try { return JSON.parse(atob(t.split('.')[1].replace(/-/g, '+').replace(/_/g, '/'))).exp || 0; } catch { return 0; } }
  function tokenValid(s) { return !!(s && s.token && s.exp && s.exp * 1000 > Date.now() + 30000); }

  function loadSession() {
    return new Promise((resolve) => {
      if (session) return resolve(session);
      try { chrome.storage.local.get([JWT_KEY], (d) => { session = (d && d[JWT_KEY]) || null; resolve(session); }); }
      catch { resolve(null); }
    });
  }
  ...
  async function signIn(interactive = true) {
    ...
    const authUrl = 'https://discord.com/api/oauth2/authorize?' + new URLSearchParams({
      client_id: DISCORD_CLIENT_ID, response_type: 'code', redirect_uri: redirectUri,
      scope: 'identify', prompt: 'consent',
    }).toString();
    const redirect = await new Promise((resolve) => {
      try { chrome.identity.launchWebAuthFlow({ url: authUrl, interactive }, (r) => resolve(chrome.runtime.lastError ? null : r)); }
      catch { resolve(null); }
    });
    ...
    try { chrome.storage.local.set({ [JWT_KEY]: session }); } catch {}
    return session;
  }
  ...
  function bindForm() {
    ...
    desc.addEventListener('input', () => { $('#desc-count').textContent = `(${desc.value.length}/500)`; });
    ...
    $('#report-form').addEventListener('submit', async (e) => {
      ...
      if (window.CSRPConfirm) {
        const ok = await window.CSRPConfirm({
          ...
          okLabel: 'Submit report', danger: true,
        });
        if (!ok) return;
      }
      const submit = $('#submit-btn');
      submit.disabled = true; msg.className = 'msg';
      showOk('Submitting…');
      try {
        const payload = { targetId, reason, description: desc.value.slice(0, 500) };
        const vf = $('#video').files[0];
        const sf = $('#shot').files[0];
        if (vf) {
          if (vf.size > VIDEO_MAX) { submit.disabled = false; return showErr('Video is over 50 MB.'); }
        ...
        if (!token) { submit.disabled = false; return showErr('Sign in required.'); }
        const resp = await pro('/report', { method: 'POST', body: payload, token, timeoutMs: 120000 });
        submit.disabled = false;
        if (resp.ok) {
          showOk('✓ Report submitted. Thank you — you can close this tab.');
          $('#report-form').reset(); $('#desc-count').textContent = '(0/500)';
        } else if (resp.status === 401) {
          showErr('Session expired — please sign in again.');
          try { chrome.storage.local.remove(JWT_KEY); } catch {} session = null; reflectAuth();
        } else {
          showErr((resp.data && resp.data.error) || resp.error || 'Could not submit report.');
        }
      } catch (err) { submit.disabled = false; showErr('Something went wrong. Try again.'); }
    });
  }

  async function render() {
    bindBack(); bindForm();
    if (!/^\d{15,21}$/.test(targetId)) { showErr('Invalid or missing target id.'); return; }
