(() => {
  ...
  function vetoButtons() {
    ...
    for (const btn of document.querySelectorAll('button[type="button"], button')) {
      ...
      const looksVeto = /maps%2Fimages%2Fde_/.test(html) || /maps\/images\/de_/.test(html) || btn.className.includes("border-theme-gray") && btn.className.includes("rounded-md");
      ...
      if (!map) continue;
      const enabled = !btn.disabled && btn.getAttribute("aria-disabled") !== "true" && !/cursor-not-allowed/.test(btn.className) && !/pointer-events-none/.test(btn.className);
      out.push({
        btn,
        map,
        enabled
  ...
  function tick() {
    ...
    const ranked = enabled.map(b => ({
      ...b,
      rank: order.indexOf(b.map) === -1 ? 999 : order.indexOf(b.map)
