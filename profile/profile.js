(() => {
  ...
  try {
    chrome.storage.local.get([ "csrpMyId" ], d => {
      if (d && d.csrpMyId) CSRP_MYID = String(d.csrpMyId);
    });
  } catch {}
  let sndCfg = {
    soundEnabled: true,
  ...
  function inPeriod(dateStr, p) {
    ...
    const y = new Date(sToday - 864e5);
    return d >= y && d < sToday;
  }
  function toRow(m, uid) {
    if (m.canceled) return null;
    const pl = m.players?.[uid];
    if (!pl) return null;
  ...
  function renderIdentity(profile) {
    ...
    card.innerHTML = '<div class="ps-holo" aria-hidden="true"></div>' + '<div class="ps-corner ps-corner-tl" aria-hidden="true"></div>' + '<div class="ps-corner ps-corner-br" aria-hidden="true"></div>' + `<div class="ps-rank" title="${esc(rankTitle)}"><span class="ps-rank-dot"></span>${esc(rankLabel)}</div>` + '<div class="ps-avwrap">' + (avatar ? `<img class="ps-av" src="${avatar}" alt="" />` : '<div class="ps-av ps-av-empty">?</div>') + '<span class="ps-avring" aria-hidden="true"></span>' + "</div>" + `<div class="ps-name">${esc(profile.name)} ${flag}</div>` + `<div class="ps-id" title="User ID">#${esc(id)}</div>` + '<div class="ps-elo"><span class="ps-elo-coin" aria-hidden="true"></span>' + `<b>${profile.points ?? "—"}</b><span class="ps-elo-lbl">ELO</span></div>` + '<div class="ps-tier-row" id="ps-tier-row"></div>' + '<div class="ps-mini">' + `<div class="ps-mini-cell"><b class="${wr >= 50 ? "good" : "bad"}">${matches ? wr.toFixed(0) + "%" : "—"}</b><span>Winrate</span></div>` + `<div class="ps-mini-cell"><b>${kdr.toFixed(2)}</b><span>KDR</span></div>` + `<div class="ps-mini-cell"><b>${matches}</b><span>Matches</span></div>` + "</div>";
    ...
    const reportBtn = isSelf ? "" : `<button class="ph-report" id="ph-report" title="Report this player to CSR+ moderators">⚑ Report</button>`;
    $("#head").innerHTML = (avatar ? `<img class="ph-av" src="${avatar}" alt="" />` : '<div class="ph-av"></div>') + `<div class="ph-info"><div class="ph-name">${esc(profile.name)} ${flag}</div>` + `<div class="ph-links">${links}${reportBtn}</div></div>`;
    const rb = $("#ph-report");
  ...
  function rating(rs, profile) {
    ...
    const kr = rounds ? kills / rounds : .68;
    const adr = rs.length ? clamp(kr * 100 + (rounds ? assists / rounds : 0) * 22, 30, 160) : 72;
    const krN = clamp((kr - .4) / .6, 0, 1);
    const kdN = clamp((kd - .5) / 1, 0, 1);
    const adrN = clamp((adr - 50) / 60, 0, 1);
    return clamp(.5 * kdN + .3 * krN + .2 * adrN, 0, 1.5) * .95 + .05;
  }
  function renderStats(profile) {
    ...
    const stats = [ [ "Winrate", winratePct.toFixed(2) + "%", true ], [ "ELO", String(profile.points ?? "—"), true ], [ "KDR", kdr.toFixed(2), true ], [ "AVG", avg.toFixed(2), true ], [ "Rating", rating(rs, profile).toFixed(2), true ], [ "Kills", String(profile.kills ?? 0) ], [ "Deaths", String(profile.deaths ?? 0) ], [ "Matches", String(profile.matches ?? 0) ], [ "Wins", String(profile.wins ?? 0) ] ];
    $("#stats-block").hidden = false;
    const grid = $("#stat-grid");
    grid.innerHTML = "";
    stats.forEach(([label, value, accent], i) => {
      const cell = el("div", "stat" + (accent ? " stat-accent" : ""));
      cell.style.animationDelay = i * .03 + "s";
  ...
  function lineChart(canvas, values, {color = "#fff", fmt = v => String(Math.round(v))} = {}) {
    ...
    const span = max - min, head = span * .12;
    min -= head;
    max += head;
  ...
  function barChart(canvas, rows) {
    ...
    rows.forEach((r, i) => {
      draw(i, .5, r.kills, "#c7ffd9");
      draw(i, 1.5, r.deaths, "#ffb3b8");
      draw(i, 2.5, r.assists, "#8ab8ff");
    });
    ctx.fillStyle = "#c7ffd9";
    ctx.fillRect(PAD.l, h - 11, 8, 8);
    ctx.fillStyle = "rgba(244,246,251,0.6)";
    ctx.fillText("K", PAD.l + 11, h - 4);
    ctx.fillStyle = "#ffb3b8";
    ctx.fillRect(PAD.l + 28, h - 11, 8, 8);
    ctx.fillStyle = "rgba(244,246,251,0.6)";
    ctx.fillText("D", PAD.l + 39, h - 4);
    ctx.fillStyle = "#8ab8ff";
    ctx.fillRect(PAD.l + 56, h - 11, 8, 8);
    ctx.fillStyle = "rgba(244,246,251,0.6)";
    ctx.fillText("A", PAD.l + 67, h - 4);
  }
  function eloSeries() {
    ...
    return {
      values,
      estimated: rows.some(r => r.eloDelta == null)
    };
  }
  function drawCharts() {
    const rows = [ ...state.rows ].reverse();
    if (!rows.length) return;
    const elo = eloSeries();
    $("#elo-sub").textContent = elo.estimated ? `estimated from W/L (±25) — last ${rows.length} matches` : `last ${rows.length} matches`;
    lineChart($("#chart-elo"), elo.values, {
      color: "#ffd24a"
    });
    lineChart($("#chart-kdr"), rows.map(r => r.deaths ? r.kills / r.deaths : r.kills), {
      color: "#8ab8ff",
      fmt: v => v.toFixed(1)
    });
    ...
    for (const r of state.rows) {
      ...
      byMap[k].games++;
      if (r.won === true) byMap[k].wins++;
      byMap[k].kills += r.kills;
      byMap[k].deaths += r.deaths;
    }
    ...
    if (maps.length) {
      ...
      for (const [m, d] of maps) {
        ...
        row.innerHTML = `<img src="${mapIcon(m)}" alt="" /><span class="mr-name">${esc(mapLabel(m))}</span>` + `<span class="mr-games">${d.games} game${d.games === 1 ? "" : "s"}</span>` + `<div class="mr-bar"><span style="width:${wr.toFixed(0)}%"></span></div>` + `<span class="mr-wr ${wr >= 50 ? "good" : "bad"}">${wr.toFixed(0)}%</span>` + `<span class="mr-kd">${kd.toFixed(2)} KD</span>`;
  ...
  function histRow(r, i) {
    ...
    row.style.animationDelay = Math.min(i * .02, .4) + "s";
    const d = new Date(r.date);
    const date = isNaN(d) ? "" : d.toLocaleDateString("en-GB", {
      day: "numeric",
      month: "short",
      year: "numeric"
    });
    const time = isNaN(d) ? "" : d.toLocaleTimeString("en-GB", {
      hour: "2-digit",
      minute: "2-digit"
    });
    const kdrRow = r.deaths ? (r.kills / r.deaths).toFixed(2) : r.kills.toFixed(2);
    const res = win ? "W" : loss ? "L" : "—";
    const score = r.myScore != null && r.oppScore != null ? `<span class="hs-res">${res}</span><span class="hs-mine">${r.myScore}</span><span class="hs-sep">:</span><span class="hs-opp">${r.oppScore}</span>` : `<span class="hs-res">${res}</span>`;
    row.innerHTML = `<div class="hc hc-date"><span>${date}</span><span class="dim">${time}</span></div>` + `<div class="hc hc-score">${score}</div>` + `<div class="hc hc-kda">${r.kills} / ${r.deaths} / ${r.assists}</div>` + `<div class="hc hc-kdr">${kdrRow}</div>` + `<div class="hc hc-map"><img src="${mapIcon(r.map)}" alt="" />` + `<span class="hm-txt"><span>${esc(mapLabel(r.map))}</span><span class="dim">${r.mode}</span></span></div>`;
  ...
  function renderHistory() {
    ...
    if (!state.rows.length) {
      list.append(el("div", "empty", "No matches on record."));
      return;
    }
  ...
  const rarity = r => RARITY[Number(r)] || {
    name: "",
    c: "#9aa0ad",
    cw: ""
  };
  ...
  function invBlob(it) {
    ...
    it.__blob = [ it.name, typeName(it.item_type), rar.name, rar.cw, w ? `${w.code} ${w.label}` : "", it.stattrak ? "stattrak st" : "", gem ? `${gem.label} gem special t1 t2 t3` : "", it.nametag || "", String(it.item_type) === "1" ? "knife knives gold" : "" ].join(" ").toLowerCase();
    return it.__blob;
  }
  function invCard(it, i) {
    ...
    c.style.animationDelay = Math.min(i * .012, .45) + "s";
    ...
    const floatBadge = w ? `<span class="c-float" style="color:${w.c};border-color:${w.c}55"><span class="c-dot" style="background:${w.c}"></span>${w.code} · ${it.float.toFixed(4)}</span>` : "";
    const seedBadge = it.seed != null ? `<span class="c-seed">#${Math.round(it.seed)}</span>` : "";
    const gemBadge = gem ? `<span class="c-gem" style="color:${gem.color};border-color:${gem.color}66" title="${esc(gem.title || gem.label)}">${esc(gem.label)}</span>` : "";
  ...
  function openItemModal(it) {
    ...
    if (it.nametag) {
      tag.hidden = false;
      tag.textContent = `Name tag: “${it.nametag}”`;
  ...
  function applyInvView() {
    ...
    if (s.sort === "rarity") list.sort((a, b) => b.rarity - a.rarity || a.name.localeCompare(b.name)); else if (s.sort === "float") list.sort((a, b) => (a.float ?? 99) - (b.float ?? 99)); else if (s.sort === "name") list.sort((a, b) => a.name.localeCompare(b.name)); else if (s.sort === "gem") {
    ...
    grid.innerHTML = "";
    if (!list.length) {
      grid.append(el("div", "empty", "No items match."));
      return;
    }
  ...
  function buildInvFilters() {
    ...
    Object.keys(counts).sort((a, b) => counts[b] - counts[a]).forEach(t => box.append(mk(String(t), typeName(t), counts[t])));
  }
  function renderInvStats() {
    ...
    $("#tb-stats").innerHTML = `<span class="kv"><b>${items.length}</b> items</span>` + `<span class="kv"><b>${knives}</b> knives</span>` + `<span class="kv"><b>${covert}</b> covert+</span>` + `<span class="kv"><b>${stat}</b> StatTrak</span>` + (gems ? `<span class="kv kv-gem"><b>${gems}</b> special patterns</span>` : "");
  }
  async function loadInventory() {
    ...
    try {
      ...
      if (dop.length) {
        console.info("[CSR+] Doppler items in this inventory:", dop.map(it => ({
          name: it.name,
          skin_index: it.skin_index,
          seed: it.seed,
          keys: Object.keys(it).join(",")
        })));
      }
    } catch {}
  ...
  function bindInvControls() {
    ...
    $("#inv-search").addEventListener("input", e => {
      clearTimeout(t);
      t = setTimeout(() => {
        state.invState.q = e.target.value.trim();
        applyInvView();
      }, 120);
    });
  ...
  async function render() {
    ...
    bindTabs();
    bindInvControls();
    $("#hist-more").addEventListener("click", () => {
      snd("click");
      loadMoreHistory();
    });
    $("#wd-x").addEventListener("click", () => {
      snd("off");
      closeItemModal();
    });
    ...
    document.addEventListener("keydown", e => {
      if (e.key === "Escape" && !$("#wd").hidden) closeItemModal();
    });
    if (!id) {
      $("#ps-card").innerHTML = '<div class="ps-skel">No player selected.</div>';
      return;
    }
    ...
    if (!profile) {
      $("#ps-card").innerHTML = '<div class="ps-skel">Could not load this player.</div>';
      return;
    }
    ...
    if (startTab !== "overview") switchTab(startTab); else drawCharts();
  }
