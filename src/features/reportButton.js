(() => {
  ...
  function tick() {
    ...
    if (anchor) {
      if (anchor.mode === 'after') anchor.node.insertAdjacentElement('afterend', makeButton(id, false));
      else anchor.node.appendChild(makeButton(id, false));
    } else {
      document.body.appendChild(makeButton(id, true));
    }
  }

  CSRP.reportButton = { tick };
})();
