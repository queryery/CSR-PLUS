(() => {
  ...
  function matchRating(r) {
    ...
    const killRating = r.kills / rounds / .679;
    const survivalRating = Math.max(0, rounds - r.deaths) / rounds / .317;
    return (killRating + .7 * survivalRating) / 1.7;
  }
  ...
  function streakOf(rows) {
    ...
    for (const r of decided) {
      if (r.won === decided[0].won) n++; else break;
    }
    return decided[0].won ? n : -n;
  }
  function aggregate(profile, rows) {
    const games = rows.length;
    const w = rows.map((_, i) => Math.pow(.92, i));
    ...
    const lifeWR = profile && profile.matches ? profile.wins / profile.matches : .5;
    const kd = games ? dpr ? kpr / dpr : kpr * 5 : lifeKD;
    const kr = games ? kpr : .68;
    const adr = games ? clamp(kr * 100 + apr * 22, 30, 160) : 72;
    const winrate = wWin != null ? (wWin * decided + .5 * 4) / (decided + 4) : lifeWR;
    ...
    const wRating = games ? rows.reduce((s, r, i) => s + matchRating(r) * w[i], 0) / wsum : 1;
    const confidence = clamp(games / WINDOW, .15, 1);
    ...
    const recentKD = recent.length ? mean(recent.map(r => r.deaths ? r.kills / r.deaths : r.kills)) : flatKD;
    const recentForm = clamp(recentKD / (flatKD || 1), .5, 1.6);
    const avgSeriesKD = mean(kdSeries) || 1;
    const peak = kdSeries.length ? Math.max(...kdSeries) / avgSeriesKD : 1;
    const elo = profile?.points ?? 1e3;
    return {
      id: profile?.id,
      name: profile?.name,
      elo,
  ...
  function strength(a) {
    ...
    const kdN = clamp((a.kd - .6) / (1.6 - .6), 0, 1);
    const krN = clamp((a.kr - .5) / (1 - .5), 0, 1);
    const adrN = clamp((a.adr - 50) / (110 - 50), 0, 1);
    const wrN = clamp((a.winrate - .3) / (.7 - .3), 0, 1);
    const formN = clamp((a.recentForm - .7) / (1.4 - .7), 0, 1);
    const ratingN = clamp(((a.rating ?? 1) - .55) / (1.45 - .55), 0, 1);
    const trendN = clamp(.5 + (a.formTrend ?? 0) * .5, 0, 1);
    const perf = .28 * kdN + .18 * ratingN + .15 * krN + .14 * adrN + .12 * wrN + .08 * formN + .05 * trendN;
    const perfWeight = .12 + .28 * (a.confidence ?? .15);
    const raw = (1 - perfWeight) * eloN + perfWeight * perf;
    const adjusted = .5 + (raw - .5) * (.72 + .28 * (a.confidence ?? .15));
    return Math.round(clamp(adjusted, 0, 1) * 100);
  }
  function playerEloAdj(p) {
    const s = strength(p);
    const perfDelta = (s - 50) * 2 * (p.confidence ?? .15);
    const momentum = (p.formTrend ?? 0) * 30 + clamp(p.streak ?? 0, -3, 3) * 7;
    return p.elo + perfDelta + momentum;
  }
  function teamElo(players) {
    if (!players.length) return 1e3;
    const adj = players.map(playerEloAdj).sort((x, y) => y - x);
    const n = adj.length;
    let num = 0, den = 0;
    adj.forEach((e, i) => {
      const w = n > 1 ? 1.2 - .4 * (i / (n - 1)) : 1;
      num += e * w;
      den += w;
    });
    return num / den;
  ...
  function winProbability(teamA, teamB) {
    ...
    const conf = (mean(teamA.map(x => x.confidence ?? .15)) + mean(teamB.map(x => x.confidence ?? .15))) / 2;
    p = .5 + (p - .5) * (.55 + .45 * conf);
